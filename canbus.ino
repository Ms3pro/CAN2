...
void receive_CAN0_message()
{
    if (!digitalRead(CAN0_INT)) {
        ...
        // Проверяем, находится ли rxId в диапазоне для ADC значений
        if (rxId >= 625 && rxId <= 672) {
            int adcIndex = rxId - 625;
            if (adcIndex < ADC_VALUES_COUNT) {
                ADC_VALUES[adcIndex] = (float)(word(rxBuf[0], rxBuf[1]));
                ADC_VALUES[adcIndex] = (float)(word(rxBuf[2], rxBuf[3]));
                ADC_VALUES[adcIndex] = (float)(word(rxBuf[4], rxBuf[5]));
                ADC_VALUES[adcIndex] = (float)(word(rxBuf[6], rxBuf[7]));            
            }
        }
switch (rxId) {
